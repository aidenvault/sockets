...
class WebSocketDemoClient {
  ...
  connect() {
    ...
    if (!url.startsWith('ws://') && !url.startsWith('wss://')) {
      this.log('ERROR', 'Invalid WebSocket URL. Must start with ws:// or wss://', 'error');
      return;
    }
    ...
    this.connectBtn.innerHTML = '<i class="ph-bold ph-spinner btn-icon" style="animation: spin 1s linear infinite;"></i> Connecting...';
    ...
    try {
      ...
      const connectionTimeout = setTimeout(() => {
        if (!this.isConnected) {
          this.log('ERROR', 'Connection timeout. Server may be down or unreachable.', 'error');
          this.ws.close();
  ...
  handleConnectionFailure() {
    ...
    this.connectBtn.innerHTML = '<i class="ph-bold ph-power btn-icon"></i> Connect';
    
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      this.reconnectAttempts++;
      this.log('INFO', `Retrying connection (${this.reconnectAttempts}/${this.maxReconnectAttempts})...`);
      setTimeout(() => this.connect(), 2000);
    } else {
      this.reconnectAttempts = 0;
      this.log('ERROR', 'Max reconnection attempts reached. Please check server status.', 'error');
    }
  }
  ...
  handleMessage(event) {
    try {
      ...
      if (message.type === 'CONNECTED') {
        ...
        this.clientIdDisplay.textContent = `ID: ${this.clientId}`;
        this.clientIdDisplay.style.display = 'block';
        ...
        this.log('ERROR', `Server error: ${message.payload.error}`, 'error');
      }
    } catch (error) {
  ...
  handleError(error) {
    ...
    this.log('ERROR', 'WebSocket connection error. Please check server status and URL.', 'error');
  }
  ...
  handleClose(event) {
    ...
    this.connectBtn.disabled = false;
    this.connectBtn.innerHTML = '<i class="ph-bold ph-power btn-icon"></i> Connect';
    this.clientIdDisplay.style.display = 'none';
  }
  ...
  updateChannelsList() {
    if (this.channels.size === 0) {
      this.channelsList.innerHTML = '<div class="empty-state">No channels joined</div>';
    } else {
  ...
  log(direction, content, type = '') {
    ...
    entry.innerHTML = `
      ...
      <span class="log-direction ${type}">${direction}</span>
      <div class="log-content-text">${this.escapeHtml(content)}</div>
    `;
   ...
   */
  clearLog() {
    this.logContent.innerHTML = '<div class="empty-state">Log cleared</div>';
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('💡 Tip: Press Ctrl+Enter (or Cmd+Enter) to send messages quickly');
});
